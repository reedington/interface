...
test('worker restart retains unknown intent and does not resume unfinished browser actions',()=>{
  ...
  try {
    ...
    store.saveRun(r);store.close();store=new Store(dir);
    const recovered=store.runs()[0];assert.equal(recovered.status,'completed');assert.equal(recovered.effect,'unknown');assert.equal(recovered.outcomeCode,'OUTCOME_UNKNOWN');assert.equal(recovered.owner,'none');assert.equal(recovered.epoch,5);assert.equal(recovered.events.at(-1)?.kind,'recovery');store.close();
